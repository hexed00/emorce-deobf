 ...
 * Body: multipart file or JSON { script: "..." }
 * Returns: { code, build, pastefy? }
 */
  ...
  try {
    ...
    if (data && data.success && data.paste) {
      return {
        ...
        raw: data.paste.raw_url || `https://pastefy.app/${data.paste.id}/raw`,
      };
    }
    return null;
  } catch (e) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!String(script).trim()) {
      res.statusCode = 400;
      res.end(JSON.stringify({ error: 'empty script' }));
      return;
    }
    ...
    const wantPaste =
      ...
      body.pastefy === '1' ||
      body.pastefy === 'true' ||
      req.headers['x-pastefy'] === '1';

    let pastefy = null;
    if (wantPaste) {
      pastefy = await uploadPastefy(code, `Emorce HEXED ${build}`);
    }
